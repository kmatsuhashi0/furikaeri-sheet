...
export function ConfirmationPage({ formData, visibleStages, onBack, onSent }: ConfirmationPageProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!printableRef.current) throw new Error('内容の取得に失敗しました。')

      setStatus('generating')
      const { generatePdfFromElement, blobToBase64 } = await import('../utils/generatePdf')
      const pdfBlob = await generatePdfFromElement(printableRef.current)
  ...
  return (
    <div className="page">
      <div ref={printableRef} className="review-printable">
